...
* For diff
- Maybe LLM is less able to make a decision about the line numbers, so if we constrained the delete line to the llm generated line number, maybe it will perform worse if we constrained the numbers to the line.
- Also we could test in parallel how llm will react if we give it the patch applied code.
